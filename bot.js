...
bot.on("message", async (msg) => {
  ...
  let firstUser =
    msg.mentions.users == null ? null : msg.mentions.users.first(); // enforce null conditions
  ...
  if (
    !(
      msg.member.roles.cache.has(process.env.officerRoleID) ||
  ...
  for (var i = 0; i < args.length; ++i) {
    ...
    await member
      .then((values) => {
        ...
        for (var pos = i + 2; pos < args.length; ++pos) {
          // Start at the position in the command list when
        ...
        switch (
          command // check the valid commands
        ) {
          case "~help":
          case "~h":
          ...
          case "~l":
            ...
            if (nick == null) {
              // if the nickname is null, the user doesn't have one
          ...
          case "~ap":
            if (getClassFromMention(args[i + 2]) == null)
              throw "Can not add a player without a class!";
            else
              addPlayer(
                raidFileName,
                commandUsr,
                getClassFromMention(args[i + 2])
              );
            break;
          case "~~remove-player":
          case "~rp":
          ...
          case "~set-file":
            ...
            throw (
              "File set for the bot as " +
              raidFileName +
              " all operations will be performed on this file unless specified otherwise."
            );
            break;
          case "~create-file":
          ...
          case "~delete-file":
            if (fs.existsSync(args[i + 1])) {
              fs.unlink(args[i + 1], function (err) {
      ...
      .catch((err) => {
        console.log(err);
        if (!(err == null)) msg.channel.send(err);
      }); // Catch any errors or messages, display to the console and the discord bot
  }
});
...
function helpMessage(usr) {
  ...
  usr.send(
    "All commands must start with the control character indicated by '~', without the quotations."
  );
  usr.send(
    "'~loot' followed by an @user and [item text] will indicate that loot had been given to said user."
  );
  usr.send(
    "'~add-player' followed by an @user and [item text] will indicate that loot had been given to said user."
  );
  usr.send(
    "'~remove-player' followed by an @user and [item text] will indicate that loot had been given to said user."
  );
  usr.send(
    "'~retract' followed by an @user and [item text] will retract the item from the user."
  );
  usr.send(
    "'~loot-history' followed by an @user or @role will display the loot history for that player or class."
  );
  usr.send("'~attendance' ");
  usr.send("'~raid-roster' will display the current raid roster.");
  usr.send(
    "'~start-raid' will begin a raid specified by the current date. It must be ended before the day is over."
  );
  usr.send(
    "'~end-raid' will end the raid specified by the current day. It must be used after the '~start-raid' command was indicated"
  );
  usr.send(
...
function createNewFile(fileName, userList, classList) {
  ...
  if (!(userList == null)) {
    if (!(userList.length == classList.length))
      // Every user MUST have a class
...
function addDate(fileName, raidDate) {
  ...
  for (let elem = 0; elem < contents.length; elem++) {
    // split the file into an array for parsing
    raidArray.push(contents[elem].split(","));
  }

  for (let find = 0; find < contents.length; ++find)
    if (raidArray[find][0] == raidDate)
...
function takeAttendance(fileName, usr, accessDate) {
  ...
  for (let find = 0; find < contents.length; ++find)
    if (raidArray[find][0] == accessDate) {
      // date located
      foundRow = find;
      break;
    }
  for (let find = 1; find < raidArray[0].length; ++find) {
    if (raidArray[0][find] == usr) {
      // user located
      foundCol = find;
      break;
    }
  ...
  else raidArray[foundRow][foundCol] = "{No Item}";
...
function addPlayer(fileName, usr, usrClass) {
  ...
  for (let elem = 2; elem < contents.length; elem++) {
    // split the file into an array for parsing
    raidArray.push(contents[elem].split(","));
  }
...
function editPlayer(
  ...
  let returnString = "";
  let foundRow = 0,
    foundCol = 0; // how many cols are there in the file
  ...
  for (find = 0; find < tempString.length; ++find) {
    if (tempString[find] == usr) {
      // redundant: but checks for user prior to adding
      break;
    }
  }
  ...
  if (find == contents.length)
    // full epoch, no attribute found
  ...
  if (isRetracted) {
    ...
    if (
      raidArray[foundRow][foundCol] == "{absent}" ||
      ...
      tempArray = raidArray[foundRow][foundCol]
        .replace("{", "")
        .replace("}", "")
        .split("/");

      for (find = 0; find < tempArray.length; ++find) {
        // iterate all items assigned to a player

        if (tempArray[find] == playerItem) break;
      }
...
function readPlayer(fileName, usr, accessDate, amountOfEntries) {
  ...
  if (amountOfEntries == "all") entries = foundRow - 1;
  // the amount of dates there are in the file
  ...
  //if()
  for (let back = foundRow; back > foundRow - entries; --back) {
    //TODO: ERROR -- On some entries it only displays one or nothing                                                     // count back from date entries and display the
    //if(back<=1) break;
...
function readClass(fileName, usrClass, accessDate, amountOfEntries) {
  ...
  for (find = 0; find < tempString.length; ++find) {
    //console.log(find);
    if (tempString[find] == usrClass) {
      // redundant: but checks for user prior to adding
      classArray.push(find); // append all the locations of the class into the arrya
    }
  }
  console.log(tempString.length - 1);
  if (classArray.length == 0) throw "No players with that class specified.";
  ...
  for (let col of classArray) {
    ...
    for (let back = foundRow; back > foundRow - entries; --back) {
      // count back from date entries and display the
